...
**여기서는 거의 안 걸린다** (D-0182). 그 표기는 `key` 문자열에 안 들어 있고
`margin` 필드를 **표시할 때** `KEY_MARGIN_FLOOR`와 견줘 붙인다 — 이 탐침이 0곡을
찍은 것은 라벨이 깨끗해서가 아니라 **내가 엉뚱한 곳을 봤기 때문이다.**

애매함은 `ingest keys`가 이미 분석한다 (D-0057 · D-0089). **여기서 다시 세지 않는다.**
"""
