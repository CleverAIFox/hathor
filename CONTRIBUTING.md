...
**셋 다 git이 진다** (D-0066). `git apply --check`가 검증, `git apply -R`이 되돌리기,
`git apply --reverse --check`가 멱등 판정이다. `make apply`가 그것을 감싼다 —
작업 트리가 깨끗한지 보고, 이미 적용됐으면 아무것도 하지 않고, 붙인 뒤 색인을 검사한다.
