...
def report_harmonic_sweep(rows: list[dict[str, object]], profile: str) -> int:
    ...
    print("\n--- 읽는 법 ---")
    print("**`고정초과`가 판정이다** (D-0201). 음수면 그 선법에서 추정이 무작위보다 낫다.")
    print("`초과`(전체)는 참고만 한다 — 강도를 올리면 경계의 곡이 선법을 갈아타고(`이동`)")
    print("분모가 움직여, 강도끼리 비교가 서지 않는다. **선법이 전 강도에서 안 바뀐 곡만")
    print("본 것이 `고정`이며 `고정바닥`이 그 짝이다.**")
    print("**`바닥`을 `고정`에 대지 않는다.** 귀무에서도 고정 집합은 훨씬 덜 애매하다 —")
    print("선법이 안 흔들린 표본만 남기면 경계가 걸러진다. 둘은 다른 모집단이다.")
    print("**바닥은 넷 다 선법별이고 프로파일별이다.** 표에 찍힌 값이 지금 쓴 것이다.")
    print("나란한조가 오르면 남은 애매함이 원리적 한계 쪽으로 이동한 것이다.")
    print("**귀무가 코퍼스 구조를 재현하지 않는다** (O-25 다섯째 줄). 디리클레는 선법")
    print("구성이 코퍼스와 다르다. 바닥은 하한이지 예측이 아니다.")
    return 0
