...
def add_parser(commands: argparse._SubParsersAction) -> None:  # type: ignore[type-arg]
    ...
    parser.add_argument("--like", type=float, default=None, help="이 홉 폴더와 같은 곡을 뽑는다")
...
def run(args: argparse.Namespace, library_root: Path) -> int:
    ...
    # **같은 곡을 뽑는다** (D-0164). 홉만 바꿔 견주려면 표본이 같아야 한다 —
    # `--limit`으로 다시 뽑으면 고르게 뽑기가 다른 곡을 고르고 **짝지은 비교가
    # 아니게 된다** (D-0113).
    if args.like is not None:
        ...
        if not wanted:
            ...
            return 1
        picked = [track for track in tracks if track.source_key in wanted]
        print(f"{source.name}와 같은 {len(picked)}곡")
    else:
        # **고르게 뽑는다** (D-0161). 앞에서 자르면 스캔 차례가 아티스트순이라 한
        # 아티스트만 나온다 — 20곡이 전부 10CM이었다.
        step = max(1, len(tracks) // args.limit) if args.limit else 1
...
def _write(
    args: argparse.Namespace, picked: list[ScannedTrack], library_root: Path, out_root: Path
) -> int:
    """고른 곡의 포락선을 쓴다.

    **홉을 폴더 이름에 적는다.** 홉이 다른 파일이 섞이면 박 추정이 조용히 틀린다 —
