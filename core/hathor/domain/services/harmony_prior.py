"""화성 도수 사전(prior)의 정보량을 잰다. **O-21(닫힘 D-0082)의 판정 장치이며 생성기가 아니다.**

### 무엇을 판정하는가

O-21(D-0082)의 접근안은 "저장된 크로마를 조성 기준으로 회전시켜 도수별 가중치로 쓴다"이다.
...
- **λ* = 0** — 참조곡이 보탤 것이 없다. O-21의 크로마 접근을 기각한다 (D-0082).
...
- **회전이 조성 추정에 의존한다.** 나란한조 혼동은 3반음 어긋난 회전이고 I가 vi로
  간다. 라벨이 없어 그 오차를 잴 수 없다 (O-22(닫힘 D-0201)). `margin` 기준으로 갈라서 본다.
