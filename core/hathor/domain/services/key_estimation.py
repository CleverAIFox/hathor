...
"""배음 감산 강도. **1004곡 실측으로 켠다** (D-0201 — D-0059를 뒤집는다).

D-0059가 **전체 지표 하나로** 기각했고 코퍼스가 장조에 크게 기울어 그 지표는
장조가 들었다. 선법으로 나누고 고정 집합 바닥에 대면 두 프로파일 모두에서
**단조가 자기 바닥 위에서 아래로 내려간다.** **1.0은 과감산이다.** 수치는
D-0201에 있다 (O-28 — 코드에 박지 않는다).
"""
...
def random_baseline_by_mode(
    ...
    **선법은 참값이 아니라 추정기가 뱉은 라벨이다.** 그래서 귀무 자료도 같은
    방식으로 — 추정된 선법으로 — 나눈다. 라벨이 없는 상태에서(O-22(닫힘 D-0201)) 이것이
    성립하는 유일한 짝짓기다.
    """
    estimates = _random_estimates(count, seed, profile=profile, harmonic=harmonic)
