...
def generate_harmony(
    ...
    **화음의 어휘와 빈도만 바뀌고 배열은 시드가 정한다.** 크로마는 곡 전체 평균이라
    ...
    ### 효과의 크기는 작다고 이미 재 놓았다

    D-0063 실측: 참조곡 조건화가 버는 것은 K-K 장조 프로파일이 갖는 대비의
    **14%**다. 우리 크로마 자체가 그 프로파일의 30% 대비밖에 없다.

    **그래서 두 참조곡의 진행이 몇 마디만 다르거나 같을 수도 있다.** 그것이 결함이
    아니라 측정된 크기다. 온도나 지수로 대비를 부풀리지 않는다 — 그렇게 하면
    정보를 늘리는 것이 아니라 잡음을 증폭하고, 귀로 판정하며 손잡이를 돌리는
    자리로 되돌아간다 (D-0058~D-0061에서 네 세션을 쓴 곳).
