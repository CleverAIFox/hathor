...
| O-49 잠재 표현 — **첫 조사 완료** (D-0180 · D-0181) | **`layer03`이 화성을 담는다** — P@10 0.2354로 MFCC 0.0665의 3.5배, 크로마 상한 0.5796의 34%. **검색에 좋은 층과 화성을 담는 층이 다르다**(`layer00`은 여섯 중 꼴찌) |
| **다음 작업** | **O-57 — 배음 강도를 선법으로 나눠 훑는다.** 단조에서 조성 추정이 무작위보다 나쁘고(D-0185) `--harmonic 0.3`이 그것을 바닥 아래로 내린다(D-0191). **O-53·O-49가 여기 걸려 있다.** **정본은 [`docs/PLAN.md`](docs/PLAN.md) §1이며 여기 다시 적지 않는다** (D-0130) |
