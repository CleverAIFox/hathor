...
설계 단일 진실 공급원은 [`docs/MASTER.md`](docs/MASTER.md)다. **문서를 먼저 고치고 코드를 고친다 (GR-0.1).**

## 현재 단계

**P0 — 기반.** 완료 조건: core 프로파일 기동 + CI 전 잡 통과 + 저장소 골격.

| 항목 | 상태 |
|---|---|
| 저장소 골격 (GR-2.1 계층) | 완료 |
| `import-linter` 계약 4종 | 완료 (4 kept / 0 broken) |
| ruff · mypy --strict · pytest | 완료 (커버리지 88%) |
| 결정적 생성 경로 (NFR-M6) | 완료 (`hathor.cli generate --dry-run`) |
| Docker Compose core 기동 | **미검증 — 본인 머신에서 확인 필요** |
| 부록 E 검증 항목 1~6 | **미착수** |
