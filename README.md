...
| **다음 작업** | **크로마 시계열 추출기 (O-32 · D-0104).** 창 길이 기준 둘을 세워 보고 둘 다 기각했다 — **최적점이 없고 꺾임점은 곡마다 다르다.** 그래서 **1초로 짧게 뽑아 두고 분석할 때 묶는다**(크로마는 시간 평균이라 `k`개 평균이 정확히 `k`배 창). **추출 한 번, 창 길이는 분석 인자** |
