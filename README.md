...
| **다음 작업** | **O-26 — 절대 눈금.** O-37이 D-0125로 닫혔고 **유지 확률은 제품에 안 넣는다** — `use_hold`가 `other-self`를 내리며 그 감소분 전체가 유효 마디 수였다(잔차 ±0.0057). **self/other는 대각선을 버렸으므로 유지를 원리적으로 못 잰다**(D-0103). 출력의 화음이 참조곡과 같은 *초*를 가는지 잴 자리가 없다 — 그것이 생기기 전에는 화성 리듬을 제품 경로에 넣을 근거가 없다. **착수 전에 무엇과 비교할지부터 정한다** (O-25). |
