...
| **다음 작업** | **O-29 출력 차이 실측 (D-0079).** 도구는 섰다 — `eval harmony-output`이 시드 1000개로 도수 히스토그램 거리를 재고 비교선 일곱을 낸다. **합성 자료에서 해상도를 확인했고 1004곡 산출물로 돌리는 것이 남았다.** 음원·GPU 불필요, 약 16초 |
...
아니라 유한 표본의 되튐이다** — 낮은 대비에서는 재는 값의 40%였다.
