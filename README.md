...
| **다음 작업** | **O-33 (D-0085).** O-31이 닫혔다 — 다이어토닉 6도수 제한이 **두 사전 거리의 44%를 버리고** 그중에 곡 고유 정보가 있다. **그런데 "곡 고유"가 "음악적"은 아니다** — 조성 추정 오차도 곡마다 다르다. `margin`과 비음계 질량의 상관으로 가른다. **그 전에는 어휘 확장에 착수하지 않는다** |
...
**`within`이 기준선이다.** 다이어토닉·비음계 조 안에서만 치환하므로 조별 질량이
정확히 보존되고 어느 칸인지만 사라진다. 실측이 `within`보다 **작으면** 곡들이 같은
비음계 자리에서 닮았다는 뜻이고 버려도 곡 정체성을 잃지 않는다.

`몫/질량`이 1보다 크면 **비음계 칸이 질량 대비 더 갈린다**는 뜻이다.
