...
| **다음 작업** | **O-32 게이트 재실측 (D-0099 · D-0100).** 겹치지 않는 `other` 대 `bass`로 잰다. **이어받기가 스템 조합 변경을 못 봐서 추출이 한 곡도 안 돌았다** — 조건에 `stem_sets`를 넣고 건너뛴 이유를 찍게 고쳤다. `ingest keys --halves --separate --limit 200` → `eval time-drift` |
