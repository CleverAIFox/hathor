...
1004곡을 **한 패스로 다시 뽑았다** (D-0203). 곡마다 MERT 13층 × 5소스 · 크로마 ·
온셋 · 음고 · 무음이 `var/ingest/audio/<해시>.npz`에 있고 **곡마다 manifest가
붙는다.** 기본값은 조성 `temperley` · 배음 `0.5` (D-0201), 템포 `126` (D-0206).

| 단계 | 상태 |
|---|---|
| P0 기반 · P1 인제스트 #1~#14 | 완료 — 상세는 `docs/MASTER.md`, 근거는 `docs/DECISIONS.md` |
| 문서 체계 | **넷** — `README`(진입) · `docs/MASTER.md`(현재) · `docs/PLAN.md`(미래) · `docs/DECISIONS.md`(과거). 색인·조각 없음 (D-0186 ~ D-0189) |
...
| O-47 박이 격자에 흔들린다 — **닫힘** (D-0173) | 원인은 **창이 홉에 묶여 있던 것**이었다(D-0169). 창을 초로 고정하니 뒤집힌 곡이 **4 / 39에서 1 / 39**로 줄었다 |
| O-49 잠재 표현 — **첫 조사 완료** (D-0180 · D-0181) | **`layer03`이 화성을 담는다** — P@10 0.2354로 MFCC 0.0665의 3.5배, 크로마 상한의 34%. **검색에 좋은 층과 화성을 담는 층이 다르다**(`layer00`은 여섯 중 꼴찌). **화성 사전 경로는 살아났고**(D-0212) 잠재 표현 조건화는 아직 없다 |
