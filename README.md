...
| **다음 작업** | **O-29 출력 차이 측정 (D-0078).** 1004곡 스템 배치를 마쳤고 생성이 조회로 돈다. **조건화 이득은 세 배가 됐는데(14% → 44%) 8마디 출력의 차이는 2마디로 그대로다** — 8마디로는 판정할 수 없다. 시드 1000개 × 도수 히스토그램 거리로 다시 잰다. 음원·GPU 불필요 |
