...
| **다음 작업** | **O-37 판정.** `use_hold=True`가 D-0113의 `other - self +0.4983`을 **올리는가 내리는가.** 내려가면 매 마디 바꾸는 제약이 거리를 누르고 있던 게 아니다 — O-38이 다른 원인을 찾아야 한다. 음성 대조는 `hold`를 전부 0으로 둔 같은 훑기다. **`.series`에서 곡별 `hold`를 뽑아 `references_from`에 물리는 CLI 경로가 먼저 필요하다** |
