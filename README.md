...
| **다음 작업** | **O-37 판정을 돌린다.** 배선은 D-0124가 했다 — `eval harmony-order --use-hold --songs 200 --seeds 200 --bars 64`이며 조건은 D-0113과 같다. `use_hold=True`가 `other - self +0.4983`을 **올리는가 내리는가.** 내려가면 매 마디 바꾸는 제약이 거리를 누르고 있던 게 아니고 O-38이 다른 원인을 찾아야 한다. **음성 대조는 `전이 있음` 선이며 같은 표에 있다.** `p = 0`인 곡(실측 4.9%)은 부분집합 표로 따로 나온다 |
