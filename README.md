...
| **다음 작업** | **npz 배선과 생성기 연결 (O-32 · D-0107).** 1004곡 시계열이 나왔고 **전이 사전 서비스가 섰다** — 부드러운 집계가 창 길이 불변을 깨서 최빈 도수로 뒤집었다. 남은 것은 (1) npz를 읽어 곡별 전이 사전을 만드는 배선 (2) 생성기가 그것을 받게 하는 것. **둘 다 CPU라 광인사에서 된다** |
