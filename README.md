...
| **다음 작업** | **O-32 창 길이 (D-0103).** 전이 지표에서 **자기 전이를 빼자 구간 길이에 완전히 불변**이 됐다(1→8구간에서 0.6063 고정). **박자 추정을 안 해도 된다** — 고정 시간 창으로 충분하다. 남은 것은 창 길이 고르기고, 너무 길면 화음이 섞이고 너무 짧으면 크로마 추정이 흔들린다 |
